...
if submit_button:
    with st.spinner('Analyzing environmental data and generating recommendation...'):
        ...
        # Ensure correct dtypes — string columns must be str, numeric must be float/int
        str_cols = [
            'AreaType', 'TrafficDensity', 'Sunlight', 'SoilType',
            'Drainage', 'SpaceType', 'AreaSize', 'MaintenanceLevel',
            'WateringPreference', 'Purpose'
        ]
        num_cols = [
        ...
        # Pipeline handles all encoding/scaling internally — no get_dummies needed
